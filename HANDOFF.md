...
1. **non-blocking-parallel python -c 字符串字面假阳治理**（2026-05-14 发现，**假阳率
   60% / 5 次 7d**）— karma 自测 `_SLEEP_RE` 类探针 `python3 -c "for c in ['sleep 5']: ..."`
   被错算真 sleep。
   - 真根因：`common._INDIRECT_SHELL_RE` 把 `python -c` 跟 `bash -c` 同样处理（保留
     扫内容），但 python 字符串字面里的 sleep 是数据不是真 shell 执行
   - fix 方向：参考 v0.4.13 deep-fix 同根因治理 — non_blocking.py 加 `_LANG_C_HEAD_RE`
     跟 bypass_karma.py 完全一致：命令头是 python/node/ruby/perl + -c 时跳 sleep
     检测。两 check 共用模式可考虑提到 common.py 暴露 `is_lang_c_command(cmd)` 工具。
2. **keep-pushing 第 3 类假阳：数字 stats / release 链接结尾被算无推进**（2026-05-14
   v0.4.12 后发现）— `audit` 视图显示 keep-pushing 26 条 100% 都是「纯陈述完结无推进」
   触发，karma 自己标 ⚠️ 可能假阳。
   - 真触发 pattern：response 末尾给「Release 链接 https://...」或「N 测试全过」
     已被识别但**链接后还有内容**（如 `https://x.com 真闭环最后一步`）— 推进信号
     检测窗口（末尾 80 字）位置不对
   - fix 方向：扩展末尾窗口 / 或者「链接 + 后续语义跟随」语义识别推进意图
3. **audit 加 fix 时间线视图**（v0.4.16 协议 fix 后的真自然延续）— 当前 audit 不
   区分「fix 前的真假阳累积」vs「fix 后的真违反」。加 `karma audit --with-fix-timeline`
   flag 调 `git log` 查每个 check 文件最新 commit 时间，对比 violation.ts 标记每条
   violation 在 fix 前 / 后。
   - 真价值：dogfooding 闭环视图 — 用户能看「我修了 v0.4.15 后 chinese-plain 假阳
     真的不再触发了」
   - 限制：仅在 karma 仓库 cwd 跑 + git 可用时启用（dev 工具，不破坏跨用户场景）
   - 实施量：cmd_audit() 加 git log 调用 + flag 参数 — 小工程
