...
附带的具体工程 bug + 修复（真双层根因）：那次 force_block 假阳的双层根因：

1. **session_id 跨 compact 不变** — Claude Code session compact 不换 session_id，
   `count_recent_turns` 已按 session_id 过滤（不是漏过滤），但因 id 同 → 过滤等于没过滤
2. **老违反无 turn 字段，`.get("turn", 0)` fallback 成 0** — 老格式（turn 维度引入前）
   写入的 violations.jsonl 没有 turn key，fallback 0 落入当前窗口（cutoff 可能 ≤ 0）

**第二层修了**：`recent_turns / count_recent_turns` 加 `if turn_raw is None: continue`,
不让无 turn 字段的老违反被当前窗口误数。回归测试 2 条
（test_recent_turns_skips_legacy_no_turn_field / test_count_...）已守护。

**第一层 karma 管不到**：Claude Code 没暴露 compact 边界信号 hook，karma 拿不到
「这条违反是 compact 之前的」这种判断。能做的只有 turn 字段语义正确（第二层修了
之后老格式被自然跳过 = 等价绕过第一层问题）。
