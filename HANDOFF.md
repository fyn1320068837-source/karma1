...
1. **non-blocking-parallel python -c 字符串字面假阳治理**（2026-05-14 发现，**假阳率
   ...
   - fix 方向：non_blocking.py 不改 common.py（怕影响 bypass_karma 拦语言层绕
     过）— 拆 `;` `&&` `||` 子命令，命令头是 python/node/ruby/perl 时跳该子
     命令 sleep 检测。或者 common.py 加 `strip_lang_c` 参数让两个 check 不同
     选择。
