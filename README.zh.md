...
### 推荐路径：`/karma rule <自然语言>`（Claude Code skill，开箱即用）

最快路径 — 用大白话描述你想要的偏好，Agent 优化成 karma 验证过的结构、预览给你看、跟你确认、然后写入。`karma init` 自动装 skill（v0.5.12+），装完就能用。

```
你（在 Claude Code）: /karma rule 我说「完成」的时候希望附测试通过证据

Agent: [查现有规则、起草 yaml、跑 karma rule preview、
        跟你确认、跑 karma rule add — 整个 30 秒搞定]
```

skill 帮你做的事：
- **语气优化** — 把「你必须 X」改成 karma 的「协作默契」语气（LLM 对这种语气反应更好）
- **重叠检测** — 跟现有规则语义冲突自动提示，给修改 / 合并 / 新加 sibling 选项而不是默默加重复
- **anchor-vs-scope 歧义识别** — 用户说「X 场景下要 Y」时主动问：X 是引发例子还是死作用域？避免默默猜
- **locale 感知** — 用户讲中文写中文 preference，讲英文写英文
- **schema + REGISTRY 校验** — schema 检查 + `violation_checks` 函数名 REGISTRY 验证后才写盘
- **modify 配方** — 修改现有规则用 `remove + add` 组合命令，不需要单独的「replace」命令

skill 源文件：[`skills/karma-rule.md`](./skills/karma-rule.md)。`karma init` 后装到 `~/.claude/skills/karma-rule.md`。升级用 `karma install-skill`（`--force` 才覆盖本地改动；不带 `--force` 时新版写到 `karma-rule.md.new` 让你先 diff）。
