...
_PATTERNS_ALL = [
    (
        ...
        "想想这个长 ID 是真特例必须 hard code，还是该提到配置 / 通用判定？真特例就明说原因；"
        "不是就把字面提到 config 让分支退化成查表。",
    ),
    (
        ...
        "自检这是真小测试样本（可保留）还是生产黑白名单（该提到配置 / 数据库）？"
        "生产名单写死代码以后维护痛 — 提到配置文件或 DB 长期更优雅。",
    ),
    (
        ...
        "全大写常量长列表通常是硬编码名单。想想这是真静态 enum（可保留）还是该跟数据"
        "走的动态列表（提到配置）？后者写死代码扩展性差。",
    ),
]
...
_PATTERNS_BASH_ONLY = [
    (
        ...
        "commit message 标题用了「quick fix / hack / temp / 临时」类字眼。想想这是真应急"
        "改动（真合理就在 body 明说原因 + follow-up 计划），还是赶进度图省事不愿写清楚"
        "（不合理就重写标题说明改了什么 + 为什么）？",
    ),
    (
...
_PATTERNS_WRITE_EDIT_ONLY = [
    (
        ...
        "代码含 TODO/FIXME/HACK 标记。想想是真留 follow-up（真合理就在 PR 描述写清楚"
        "计划），还是知道该改但偷懒留着？后者就直接修。",
    ),
    (
        ...
        "注释含「打补丁 / 绕过 / 凑数 / workaround」意图。自检：是真情境合理（等上游修 / "
        "短期发版前过渡），还是赶进度图省事？合理就在注释里明说为什么是临时；不合理就"
        "找根因方案再提交。",
    ),
]
