"""karma i18n — Simple yaml-dict translation lookup.
...
suggested_fix, CLI output) goes through ``tr(key)`` lookup. Locale resolved:
  ...
  config.yaml ``locale`` field:
    ...
    - "zh" → Chinese
    - "auto" → ``karma.locale_detect.is_chinese_user()`` decides
...
def _resolve_locale() -> str:
    """Resolve effective locale from config + KARMA_LOCALE override + auto-detect."""
    # Env var has highest priority (for testing / one-off override)
    env_override = os.environ.get("KARMA_LOCALE", "").strip().lower()
    if env_override in ("en", "zh"):
        return env_override

    # Read config
    try:
        from karma.config import load as _load_cfg
        cfg = _load_cfg()
