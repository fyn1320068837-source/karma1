...
- **v0.4.44 SubagentStop + PreCompact schema 真合规（系统性 schema bug 根治）** — v0.4.43 fix Stop 后子 Agent 调研真发现：v0.4.x 早期阶段 3 个「stop 类」hook（Stop / SubagentStop / PreCompact）都错用 `hookSpecificOutput.additionalContext` — 协议不支持，长期被 Claude Code 静默拒绝。本版本 fix 剩下 2 个（subagent_stop.py + pre_compact.py 删 hookSpecificOutput → `{}` passthrough，side effect 保留）。SessionStart / SubagentStart 真支持不动。教训：不要被一个 fix 满足，深挖系统性问题。
- **v0.4.43 Stop hook schema 真 bug fix + 语气收尾 + sticky keyword 假阳治理** — 用户报 Stop hook 输出 `hookSpecificOutput.additionalContext` 被 Claude Code 报 `Expected schema` 错（Stop 协议不支持该字段）— v0.4.x 早期设计错误。删 stop.py 幽灵代码改 `{}` passthrough；stop.py reason 文本同步合作默契语气；SessionStart / SubagentStart / SubagentStop 3 个 hook 注入文本同步；sticky #1 + #2 violation_keywords 假阳收紧（「硬编码」名词类 → 「我先硬编码」一人称行动声明 / 「等子 Agent」描述依赖 → 「我先等子 Agent」）。
